...
#pragma warning restore CS8618
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        {
            var results =
                await LoginService.ValidateLoginAsync(async () => await SessionStorage.GetAsync<string>("token"));
    ...
    protected async Task Approved(int msgId)
    {
        ...
        await AccountService.ExecuteReservationAsync(msg.IdentifierId.Value);

        await NotifyService.ApproveReservation(msg.Sender.Id, $"Godkänt, '{msg.Message}'");

        await NotifyService.RemoveMessageAsync(msg);
        StateHasChanged();
    }
    ...
    protected bool IsRemovalDisabled(MessageModel msg)
    {
        return msg switch
        {
            ...
            { Type: MessageType.DeniedBuy or MessageType.ApprovedBuy } => false,
    ...
    protected bool IsActionsDisabled(MessageModel msg)
    {
        return msg switch
        {
            ...
            { Type: MessageType.ApprovedBuy } => true,
            { Sender: var sender } when sender.Id == _state.User.Id => true,
